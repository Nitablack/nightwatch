module.exports = {
    ...
    'Search for car': function (browser) { // первый ТК на первый поиск
        browser 
            .page.shop() //ссылка на page    
            .search('car') // ввод значения в поисковую строку и нажатие кнопки ENTER            
            .waitForElementVisible('@welcome','Заголовок загружен')
            .waitForElementVisible('@cardBody','Результат поиска загружен')
            .assert.urlContains('search=car')
            .assert.titleContains('TestGym', 'title ok')
            .assert.textContains('@cardBody', 'car', 'car найден');
        browser 
            .page.shop() 
            .section.topMenu 
